...
- (void)setDay:(NSInteger)day
{
    ...
    self.titleLabel.text = [NSString stringWithFormat:@"%lu年%lu月",(long)_year,(long)_month];
}
...
- (void)setFirstWeekday:(NSInteger)firstWeekday
{
    ...
    if (firstWeekday > 7 || firstWeekday < 1)
        firstWeekday = 7;
    NSDictionary *weekDic = [NSDictionary dictionaryWithObjects:@[@"日",@"一",@"二",@"三",@"四",@"五",@"六"] forKeys:@[@(7),@(1),@(2),@(3),@(4),@(5),@(6)]];
...
#pragma mark 根据年月显示签到记录
-(void)showSignIn:(NSMutableArray*)items withYear:(NSInteger)year withMonth:(NSInteger)month
{
    [self.dateDtaArray removeAllObjects];
